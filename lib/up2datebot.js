...
Up2DateBot.prototype._isMentioningStatus = function (message) {
    ...
    if(text.indexOf('add') > -1) {
         ...
         var artifactId = null;
         var version  = null;
         if (this._containsGroup(text) && this._containsArtifact(text) && this._containsVersion(text)) {
                 var inputs = text.split(" ");
                 for (var i = 0, l = inputs.length; i < l; i++) {
                     if (this._containsGroup(inputs[i])) {
                         groupId = inputs[i].replace(this.groupKey, '');
                         ...
                         artifactId = inputs[i].replace(this.artifactKey, '');
                     } else if (this._containsVersion(inputs[i])) {
                         version = inputs[i].replace(this.versionKey, '');
                     }
                 }
         }

          if (groupId != null && artifactId != null && version != null) {
                currentLibraries[groupId + ":" + artifactId] = version;
                self.postMessageToChannel(channel.name, "Added groupId:" + groupId + " artifactId:" + artifactId + " version:" + version, {as_user: true});
                self.postMessageToChannel(channel.name, "Current libraries", {as_user: true});
                console.log(currentLibraries)
                for(var artifact in currentLibraries){
                   self.postMessageToChannel("GroupId:ArtifactId:" + artifact + " Version:" + currentLibraries[artifact], {as_user: true});
                }
          } else {
                 self.postMessageToChannel(channel.name, 'Please provide groupId and artifactId and version!', {as_user: true});
          }
    }
};
...
Up2DateBot.prototype._containsVersion = function(source) {
    return source.indexOf(this.versionKey) > -1
};
